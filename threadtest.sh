...
echo FIPY_SOLVERS=$SOLVER /data/guyer/miniconda3/envs/$CONDAENV/bin/mpiexec -n 1 smt run --tag thread3 -n $NCPUS --main threadtest.py params.yaml nthreads=$NTHREADS ncpus=$NCPUS nslots=$NSLOTS solver=$SOLVER $@
